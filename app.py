...
def qrcode(name):
    ...
    with col3:
        ...
        # df.to_excel("datasheet.xlsx", index=False)
                ...
                # dictionary['User']=st.session_state["name"]
                # dictionary['Status']="Received"
                # dictionary['Date-Time']=datetime.now()
                # df= df.append(dictionary,ignore_index=True)
            #df= df.append({'User':st.session_state["name"], 'Status': "Received",'Date-Time': datetime.now()},ignore_index=True)
